...
export default function RoomSlider() {
      ...
      return (
        ...
        >
          {zones.map((zone, zoneIndex) => {
            const amenitiesChunks = zone.amenities ? chunkArray(zone.amenities, 6) : [[]];
            ...
            return amenitiesChunks.map((chunk, chunkIndex) => (
              <div key={`${zoneIndex}-${chunkIndex}`} className="flex flex-col gap-8">
                ...
                <div className="mx-auto flex h-full w-11/12 flex-wrap justify-center gap-4 align-middle lg:flex-nowrap">
                  {chunk.map((amenity, index) => {
                    const icon = houseAmenities(amenity as TypeAmenity);
                    return (
                      <div
                        className="group flex w-[90px]  flex-col items-center justify-center align-middle"
                        key={index}
                      >
  ...
  return (
    <div className="mb-32 mt-10 h-48 min-h-[300px] rounded-xl border lg:mt-10 lg:min-h-[200px]">
      <Comodities />
    </div>
  );
}
