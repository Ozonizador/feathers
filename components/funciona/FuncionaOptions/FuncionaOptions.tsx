...
const FuncionaOptions = () => {
  return (
    <section className="my-20 mx-5 flex flex-col justify-center gap-5 lg:mx-32 lg:flex-1 lg:flex-row lg:gap-12">
      <div className="flex flex-col rounded-2xl bg-white p-5 drop-shadow-2xl lg:w-1/3">
        <div>
          <h1 className="mt-4 mb-9 text-center text-2xl font-bold">Gestão da casa premium</h1>
          {optionsCard1.map((option, index) => {
            return (
              <div key={index} className="my-5 flex flex-row gap-3">
                <div className="my-auto flex w-1/12">
                  <Image className="object-scale-down" src="/images/tick.png" alt="" height="25" width="25"></Image>
                </div>
                <p className="my-auto w-9/12 text-xl lg:text-lg">{option.text}</p>
              </div>
            );
          })}
        </div>
          ...
          {optionsCard2.map((option, index) => {
            return (
              <div key={index} className="my-5 flex flex-row gap-3">
                ...
                <p className="my-auto w-9/12 break-words text-xl lg:text-lg">{option.text}</p>
              </div>
            );
          })}
        </div>
